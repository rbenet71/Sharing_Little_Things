...
class Ocr_Dascam():
    def __init__(self,dir_to_find=None,oem=None,psm=None,parent=None,filter_image_selected='Dilate'):       
        ...
        file_txt=open(path.join(self.dir_to_find,'Result_OCR_Scan_'+filter_image_selected+'.txt'),'w')
        ...
        for file in scandir(self.dir_to_find):
            ...
            if ext.upper()=='.JPG' or ext.upper()=='.JPEG' or ext.upper()=='.PNG':
                # Update date Creation of JPG
                capture_time_sec_crea=path.getctime(file) # Creation Date
                capture_time_sec_modi=path.getmtime(file) # Modification Date
                if capture_time_sec_crea<capture_time_sec_modi: # Sometimes creation date is wrong
                    capture_time_sec=capture_time_sec_crea
                else:
                    capture_time_sec=capture_time_sec_modi
                ...
                day=' No Encontrado '+'.'.join(str(x).strip() for x in read['text'])
                #print(file.path,read['text'])

                for i in range(len(read['text'])):
                    if read['text'][i]=='GARMIN':
                        if len(read['text'])>i+4:
                            ...
                            try:
                                ...
                                break
                            if lat>-90 and lat<90 and lon>-180 and lon<180 and not str(lat).strip()=='' and not str(lon).strip()=='':
                                self.geotag_photo(file.path,lat,lon)
                                self.changeFileCreationTime(file.path,capture_time_sec)
                                find=True
                                incorrect-=1
                                correct+=1
                            break
                ...
                text_result=file.path+", "+day+", "+hour+",    GPS:,"+str(lat)+","+str(lon)+","+str(find)
    ...
    def changeFileCreationTime(self,fname, newtime):
